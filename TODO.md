...
- (Done, full audit + follow-up) 2026-06 file-by-file + explicit cleanup: dummy cleanup.sh + all call sites removed; consolidated duplication (arb-tx-tools MCP modules now imported by .grok direct simulator/abicoder/log-tailer for AnvilManager/abi-registry/LogCapture); README/AGENTS/llms.txt/.env.example rewritten current; trimmed graphify-out entirely + debug_*.log + test_bearer* + test_pad + old superpowers/plans/specs + optimization reports + GEMINI + empty docs/; TODO restored + updated. All type/test green, no broken refs to removed cleanup.
