...
const reservesAbi = parseAbi(['function getReserves() external view returns (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast)']);
const slot0Abi = parseAbi(['function slot0() external view returns (uint160 sqrtPriceX96, int24 tick, uint16, uint16, uint16, uint8, bool)']);
...
async function main() {
  ...
  const r0 = r[0]; const r1 = r[1];
  const a0 = rApe[0]; const a1 = rApe[1];
  ...
  const feeNum = 997n; const feeDen = 1000n; // 0.3%
  ...
  function v3SwapExactIn(amountIn: bigint, sqrtPrice: bigint, liquidity: bigint, zeroForOne: boolean): { amountOut: bigint, newSqrtPrice: bigint } {
  ...
  console.log('Net profit estimate:', (maticBackEst - amountIn) > 0n ? 'PROFIT' : 'LOSS',
    Number(maticBackEst - amountIn) / 1e18, 'MATIC');
}
